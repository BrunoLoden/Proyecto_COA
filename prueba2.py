...
from Datos_Lineas          import  slack, BusE, BusG, BusH, L,BusL,  x, Llim,D,BusD,LB, cond,sub,b
...
model.Q_Gt = pye.Var(G,T,bounds=(-9.99,9.99))
...
def bound_Pij(model,l,t):#(Funcion de Regla)
    return (-Llim[l],Llim[l])
model.Pijde = pye.Var(L,T,bounds=bound_Pij)

model.Pijpara = pye.Var(L,T,bounds=bound_Pij)
model.Qijde = pye.Var(L,T,bounds=(-9.99,9.99))

model.Qijpara = pye.Var(L,T,bounds=(-9.99,9.99))

#Definimos variable de angulo
model.Delta = pye.Var(N,T,bounds=(-1.5,1.5),initialize=0)#Radianes

for t in T:
    model.Delta[slack,t].fix(0)
model.Delta.pprint()

model.Pr = pye.Var(N,T,domain=pye.Reals)
model.Tension = pye.Var(N,T, bounds=(0.95, 1.05),initialize=1)

for t in T:
    for n in N:
        for g in G:
            if n==BusG[g]:
                model.Tension[n,t].fix(1)
        for h in H:
            if n==BusH[h]:
                model.Tension[n,t].fix(1)       
                
#model.Tension.pprint()
...
#=============================================================================
def regla_FuljoPde(model,l,t):#(Funcion de Regla)
    i,j = BusL[l]#Se usa "i,j" por que "bus_L" llamara auna tupla
    return model.Pijde[l,t] == model.Tension[i,t]**2-model.Tension[i,t]*model.Tension[j,t]*cond[l]*pye.cos(model.Delta[i,t]-model.Delta[j,t])-model.Tension[i,t]*model.Tension[j,t]*sub[l]*pye.sin(model.Delta[i,t]-model.Delta[j,t])

model.rest_FlujoPde = pye.Constraint(L,T,rule = regla_FuljoPde)

def regla_FuljoPpara(model,l,t):#(Funcion de Regla)
    i,j = BusL[l]#Se usa "i,j" por que "bus_L" llamara auna tupla
    return model.Pijpara[l,t] == model.Tension[j,t]**2-model.Tension[i,t]*model.Tension[j,t]*cond[l]*pye.cos(model.Delta[j,t]-model.Delta[i,t])-model.Tension[i,t]*model.Tension[j,t]*sub[l]*pye.sin(model.Delta[j,t]-model.Delta[i,t])

model.rest_FlujoPpara = pye.Constraint(L,T,rule = regla_FuljoPpara)

def regla_FuljoQde(model,l,t):#(Funcion de Regla)
    i,j = BusL[l]#Se usa "i,j" por que "bus_L" llamara auna tupla
    return model.Qijde[l,t] == -(sub[l]+b[l])*model.Tension[i,t]**2-model.Tension[i,t]*model.Tension[j,t]*pye.sin(model.Delta[i,t]-model.Delta[j,t])*cond[l] + model.Tension[i,t]*model.Tension[j,t]*sub[l]*pye.cos(model.Delta[i,t]-model.Delta[j,t])

model.rest_FlujoQde = pye.Constraint(L,T,rule = regla_FuljoQde)

def regla_FuljoQpara(model,l,t):#(Funcion de Regla)
    i,j = BusL[l]#Se usa "i,j" por que "bus_L" llamara auna tupla
    return model.Qijpara[l,t] == -(sub[l]+b[l])*model.Tension[j,t]**2-model.Tension[i,t]*model.Tension[j,t]*pye.sin(model.Delta[j,t]-model.Delta[i,t])*cond[l] + model.Tension[i,t]*model.Tension[j,t]*sub[l]*pye.cos(model.Delta[j,t]-model.Delta[i,t])

model.rest_FlujoQpara = pye.Constraint(L,T,rule = regla_FuljoQpara)
...
def Balance_energia(model,t,n):#(Funcion de Regla)
    return  sum(model.P_Gt[g,t]/100 for g in G if (n == BusG[g]))\
                        + sum(model.P_Gh[h,t]/100 for h in H if (n == BusH[h]))\
                        - sum(LB[d,t]/20/100         for d in D if (n == BusD[d]) )\
                        -sum(model.Pijde[l,t]    for l in L if (n == BusL[l][0]))\
                        -sum(model.Pijpara[l,t]    for l in L if (n == BusL[l][1])) + model.Pr[n,t]== 0
model.Balance = pye.Constraint(T,N, rule = Balance_energia)

def Balance_Q_energia(model,t,n):#(Funcion de Regla)
    return  sum(model.Q_Gt[g,t] for g in G if (n == BusG[g]))\
                        -sum(model.Qijde[l,t]    for l in L if (n == BusL[l][0]))\
                        -sum(model.Qijpara[l,t]    for l in L if (n == BusL[l][1])) == 0
model.BalanceQ = pye.Constraint(T,N, rule = Balance_Q_energia)
...
model.obj_t = pye.Objective(expr = \
                    sum(model.P_Gt[g,t]*bg[g]+cg[g] for g in G for t in T)+sum(90000000000*model.Pr[n,t] for n in N for t in T))
...
df=pd.DataFrame(columns=["G. Térmica","G. Hidraulica","P. Racionamiento","Demanda","Flujo De","Flujo Para","Angulo","Tension"],index=range(24))

#Para t=1

for n in N:

    df.iloc[n-1,0]=sum(pye.value(model.P_Gt[g,1]/100) for g in G if n == BusG[g])

    df.iloc[n-1,1]=sum(pye.value(model.P_Gh[h,1]/100) for h in H if n == BusH[h])

    df.iloc[n-1,2]=pye.value(model.Pr[n,1])

    df.iloc[n-1,3]=sum(LB[d,1]/20/100 for d in D if n == BusD[d])


    df.iloc[n-1,4] = sum(pye.value(model.Pijde[l,1]) for l in L if n == BusL[l][0])
    
    df.iloc[n-1,5] = sum(pye.value(model.Pijpara[l,1]) for l in L if n == BusL[l][1])
    
    df.iloc[n-1,6] = pye.value(model.Delta[n,1]) 
    df.iloc[n-1,7] = pye.value(model.Tension[n,1]) 
df.to_excel("ResultadosBarras.xlsx")


flujos=pd.DataFrame(columns=["Desde","Hasta","Flujo De","Flujo Para"], index=L)

for l in L:
    flujos.loc[l,"Desde"]=BusL[l][0]
    flujos.loc[l,"Hasta"]=BusL[l][1]
    flujos.loc[l,"Flujo De"]=pye.value(model.Pijde[l,1])
    flujos.loc[l,"Flujo Para"]=pye.value(model.Pijpara[l,1])
